...
10. **`resources/catalogs/color_catalog.v1.json` (颜色Token库)**: 标准 `color_id` 词典，统一色彩空间与编码。
11. **`resources/catalogs/renderer_bindings.v1.json` (渲染器绑定库)**: `material_id -> renderer asset` 对照层（当前基线为 D5）。
12. **`resources/catalogs/real_world_bindings.v1.json` (现实材质绑定库)**: `material_id -> 现实规格/产品` 对照层（可落地采购语义）。
13. **`resources/schemas/apr-output.schema.json` (输出契约)**: 固定 JSON 输出结构定义，确保机器可校验。
...
> - 缺失时输出应仅为澄清问题，**不得输出 Final Prompt**。
...
### 6. 🧩 Token Binding (材质/颜色ID绑定)

在生成最终输出前，必须完成 ID 绑定：

* **Material Token**: 所有主材/次材都必须映射到 `material_id`（来自 `material_catalog.v1.json`）。
* **Color Token**: 所有显式颜色都必须映射到 `color_id`（来自 `color_catalog.v1.json`）。
* **Renderer Binding**: 每个 `material_id` 至少附带一个主渲染器绑定（来自 `renderer_bindings.v1.json`，当前仅允许 `d5`）。
* **Real-world Binding**: 每个 `material_id` 至少附带一个现实规格绑定（来自 `real_world_bindings.v1.json`）。

> **HARD RULE**:
> - 禁止输出“仅自然语言材质名、无ID”的结果。
> - 若找不到绑定项，允许 `asset_id=null`，但必须提供 `lookup_key`，并标记 `binding_status`。
> - ID 只能从 catalog 选取；不得临时自造未登记ID。

## 📤 Output Format

请始终输出**单一 JSON 对象**（固定合同），并满足以下约束：

1. **Schema约束（强制）**: 必须符合 `resources/schemas/apr-output.schema.json`。
2. **Token约束（强制）**:
   - `material_id` 仅可来自 `resources/catalogs/material_catalog.v1.json`
   - `color_id` 仅可来自 `resources/catalogs/color_catalog.v1.json`
3. **绑定约束（强制）**:
   - `renderer_bindings` 来自 `resources/catalogs/renderer_bindings.v1.json`
   - `real_world_bindings` 来自 `resources/catalogs/real_world_bindings.v1.json`
4. **状态约束（强制）**:
   - Mandatory 缺失 -> `status = ASK_USER`，且 `questions` 非空。
   - Narrative/Physics 不通过 -> `status = REVISE` 或 `BLOCK`。
   - 可写实放行 -> `status = READY`，且 `questions = []`。

最小字段骨架如下：

```json
{
  "schema_version": "apr-json-1.1",
  "status": "READY",
  "request_id": "string",
  "mandatory_check": {
    "pass": true,
    "missing": [],
    "sources": {
      "lighting_source": "USER",
      "core_material_source": "USER"
    }
  },
  "gate_report": {
    "narrative": {
  ...
  "tokens": {
    "materials": [
      {
        "material_id": "mat.concrete.board_formed.raw_gray.v1",
        "default_color_id": "col.ral.7043.v1",
        "renderer_bindings": [
          {
            "engine": "d5",
            "asset_pack": "d5-library",
            "lookup_key": "board formed concrete raw",
            "asset_id": null
          }
        ],
        "real_world_bindings": [
          {
            ...
            "search_key": "board-formed concrete wall",
            "manufacturer": null,
            "collection": null,
            "sku": null
          }
  ...
  "prompt": {
    "positive_en": "string",
    "negative_en": "string",
    "render_profile_id": "render.d5.photoreal.exterior.v1"
  },
  "questions": []
}
```
...
User: "黄昏，木材"
Agent: (Generates JSON output with `mat.wood.oak.warm_cladding.v1` + bound renderer/real-world references)
